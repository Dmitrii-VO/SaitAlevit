...
export function validateNameInput(input, showError = false) {
  ...
  const isValid =
    nameValue.length >= 2 && /^[А-Яа-яЁёA-Za-z\s\-]+$/.test(nameValue);
    ...
    if (isValid) {
      ...
      if (showError && errorElement) {
        if (digitsOnly.length >= 5) {
          shakeElement(input);
          errorElement.textContent =
            "Укажите полный номер телефона (11 цифр, например: +7 (900) 123-45-67)";
 ...
 */
async function handleCTAFormSubmit(
  form,
  submitButton,
  submitText,
  submitLoading,
  ...
  try {
    ...
    if (result.success) {
      ...
      form
        .querySelectorAll(".cta__form-checkbox.invalid")
        .forEach((checkbox) => {
      ...
      showNotification(
        result.error ||
          "Произошла ошибка. Пожалуйста, попробуйте ещё раз или позвоните нам.",
        "error",
      );
    }
  } catch (error) {
    ...
    showNotification(
      "Произошла ошибка при отправке. Пожалуйста, попробуйте ещё раз или позвоните нам.",
      "error",
    );
  } finally {
...
export function initCTAForm(form) {
  ...
  const updateProgress = () =>
    updateFormProgress(progressBar, progressContainer, formInputs);
  ...
  if (areaInput) {
    ...
    areaInput.addEventListener("blur", () =>
      validateAreaInput(areaInput, true),
    );
  }
...
function calculateHousePrice(area, type, floors, finish) {
  ...
  const pricePerM2 =
    CALCULATOR_PRICES[type]?.[floors]?.[finish] ||
...
export function initCalculator() {
  ...
  function updatePrice() {
    ...
    const typeRadio = calculatorForm.querySelector(
      'input[name="type"]:checked',
    );
    const floorsRadio = calculatorForm.querySelector(
      'input[name="floors"]:checked',
    );
    const finishRadio = calculatorForm.querySelector(
      'input[name="finish"]:checked',
    );
