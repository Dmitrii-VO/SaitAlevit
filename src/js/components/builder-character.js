...
class BuilderCharacter {
  ...
  handleMouseMove(e) {
    ...
    if (this.lastMouseTime > 0 && timeDelta > 0) {
      ...
      if (
        this.currentState === STATES.HANGING &&
        speed > CONFIG.FAST_MOUSE_THRESHOLD
      ) {
        this.triggerFalling();
      }
    }
  ...
  updateFrameImage() {
    ...
    const imagePath = `/images/builders/builder-running-${frameNumber}.webp`;
  ...
  handleRunningState(distance) {
    ...
    if (this.mouseWasMoved) {
      ...
      // Затем проверяем близкое расстояние (подготовка к прыжку), но только если нет cooldown
        ...
        distance <= CONFIG.JUMP_PREPARE_DISTANCE &&
        this.jumpCooldown === 0
      ) {
